...
int
dealer_to_internal (player_t * player, const char *command)
{
  ...
  if (strncmp (command, "play?", 5) == 0)
	...
	{
	  if (strategy[PAIR][player->current_hand->cards->value]
	      [blackjack.dealer_hand->cards->value] == 'y')
	    {
	      response = split;
	    }
	}
      ...
      if (response == NULL)
	{
	  ...
	  snprintf (buffer, 16, "%c",
		    strategy[type][count][blackjack.dealer_hand->cards->
					  value]);
	  response = buffer;
	}
...
int
player_from_internal (player_t * player, char *buffer)
{
